...
class AdresseQlService(@Autowired val adresseRepository: AdresseRepository) {
    ...
    fun all() = adresseRepository.findAll().mapNotNull { mapper.toAdresseQl(it) }.toList()
    ...
    fun firstWith(content : String) : Flux<AdresseQl> {
        return Flux.interval(Duration.ofSeconds(1)).flatMap {
            val adresse = adresseRepository.findFirstByLibelleContaining(content)?.let { mapper.toAdresseQl(it) }
            Mono.justOrEmpty(adresse)
        }
    }
}
